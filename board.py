...
solver = __import__('solver').Solver()
...
class BoardView(QtGui.QTableView):
    ...
    def initView(self, cellSize):
        ...
        factor = (cellSize - 6) / fm.width('99') # рассчитываем, что в числах не будет больше 2 цифр
    ...
    def eventFilter(self, target, event): # target - viewport
        ...
        if event.type() == QtCore.QEvent.MouseButtonPress:
            if event.button() == QtCore.Qt.LeftButton:
                switchCell(event, SPACE if event.modifiers() == QtCore.Qt.ShiftModifier else BOX) # LeftClick -> box; Shift + LeftClick -> space
                return True
            elif event.button() == QtCore.Qt.RightButton:                
                model = self.model()
                boardRow = self.rowAt(event.y()) - model.board.maxColumnBlocks
                boardColumn = self.columnAt(event.x()) - model.board.maxRowBlocks
                if boardRow >= 0 and boardColumn >= 0:
...
class BoardViewItemDelegate(QtGui.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if row < board.maxColumnBlocks and column < board.maxRowBlocks: 
            ...
            if row < board.maxColumnBlocks: # это ячейка зоны чисел колонок
                ...
                self.drawNumber(painter, option.rect, number, boardRow, boardColumn)
    ...
    def drawNumber(self, painter, rect, number, row, column):
        ...
        if number:
            ...
            painter.drawText(rect, QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter, str(number))
...
class Board():
    ...
    def setData(self, row, column, newState):
        ...
        index = self.model.index(row + self.maxColumnBlocks, column + self.maxRowBlocks)
    ...
    def load1(self, filePath): # загрузить файл с цифрами как здесь: http://www.bestcrosswords.ru/jp/20003009-form.html
